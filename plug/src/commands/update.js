...
export async function runUpdate(name, options = {}) {
  ...
  if (!result) {
    console.log(chalk.yellow(`'${name}': vault '${record.vault}' is not accessible. Skipping.`));
  ...
  if (compareSemver(latestVersion, installedVersion) <= 0) {
    console.log(chalk.dim(`${name}: already up to date (${installedVersion})`));
  ...
  console.log(chalk.green(`Updated ${name}: v${installedVersion} → v${latestVersion}`));
