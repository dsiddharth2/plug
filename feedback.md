...
**Doer:** fixed in commit 8169baa — no change needed, already passing.
...
**Doer:** fixed in commit 8169baa — Task 1.1 "Done when" now includes: `npm view plugvault` returns 404 (name available) and chalk/ora import successfully under ESM. Risk register mitigation updated to reference Task 1.1 explicitly.
...
**Doer:** fixed in commit 8169baa — Task 3.2 split into Task 3.2a (basic install: parse, resolve, fetch, download, route, track) and Task 3.2b (advanced: vault prefix, conflict prompt, -g global flag, overwrite prompt, auto-init). Task 6.2 was expanded with concrete error specifications rather than split (see finding #9 annotation).
...
**Doer:** fixed in commit 8169baa — removed `-i` flag from Task 3.2a and 3.2b titles/descriptions, VERIFY Core Commands, and all other PLAN.md references. Command is now `plug install <name>` throughout.
...
**Doer:** fixed in commit 8169baa — Risk Register row updated from "ora 7.x" to "ora 8.x".
...
**Doer:** fixed in commit 8169baa — added Task 2.0 "Test infrastructure setup" at the start of Phase 2. Sets up vitest, tests/ directory, and `npm test` script. Done when `npm test` passes with a trivial test.
...
**Doer:** fixed in commit 8169baa — added "Commit Strategy" section at the bottom of PLAN.md with a table mapping each phase to a suggested commit message and the 1-commit-per-phase rule (up to 2–3 for large phases).
